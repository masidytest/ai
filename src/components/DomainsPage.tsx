  ...
  return (
    <div className="w-full py-8 px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col md:flex-row gap-4 md:gap-6">
        {/* Domain List */}
        <div className={`${selectedDomain ? "w-full md:w-1/3" : "w-full"} transition-all`}>
              ...
              <div className="bg-app-card border border-app-border rounded-xl overflow-hidden h-full">
                ...
                <div className="flex border-b border-app-border px-3 md:px-5 overflow-x-auto">
                  {(["overview", "dns", "ssl", "settings"] as const).map((t) => (
                    <button key={t} onClick={() => setActiveTab(t)} className={`px-3 md:px-4 py-2.5 text-xs font-medium capitalize transition-colors border-b-2 -mb-px whitespace-nowrap ${activeTab === t ? "text-app-accent-text border-app-accent" : "text-app-text-muted border-transparent hover:text-app-text"}`}>{t === "dns" ? "DNS" : t === "ssl" ? "SSL" : t}</button>
                  ))}
                </div>
